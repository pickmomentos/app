...
/*global AdMob */
angular.module('main')
.service('AdMobService', function ($rootScope, $localStorage, $log) {
  ...
  var onAdLoaded = function () {
    ...
    $log.log('[' + TAG + '] onAdLoaded: ' + isInterstitialReady);
  }

  var onAdPresent = function () {
    $localStorage.lastAdTimestamp = new Date().getTime();
    $log.log('[' + TAG + '] onAdPresent');
    $log.log('[' + TAG + '] lastAdTimestamp = ' + $localStorage.lastAdTimestamp)
  }
  ...
  var requestInterstitial = function () {
    ...
    if (window.cordova && AdMob && interstitialId) {
      ...
      $log.warn('[' + TAG + '] Unsupported platform');
    }
  }
  ...
  return {
    ...
    canShowInterstitial: function () {
      if (window.cordova && AdMob && isInterstitialReady) {
        ...
        $log.log('[' + TAG + '] Ad showed ' + diff + ' minutes ago.');
    ...
    showInterstitial: function () {
      if (window.cordova && AdMob) {
        ...
        $log.warnwarn('[' + TAG + '] Unsupported platform');
      }
    }
  };
});
