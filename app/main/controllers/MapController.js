...
.controller('MapCtrl', function ($scope, $rootScope, $ionicLoading, $ionicModal,
  ...
  Toast, Places, AdMobService, Geolocation, Dialog, $log) {
  ...
  var setMapZoomToFit = function () {
    ...
    for (var i = 0; i < $scope.places.length; i++) {
      ...
      var position = setPosition(place.gpslat, place.gpslon);
  ...
  var setPlaces = function (places) {
    ...
    for (var i = 0; i < places.length; i++) {
      ...
      // icon = {
      //   url: 'http://www.grupo-gestion.com.ar/themes/website/img/map-marker.png',
      //   size: {
      //     width: 32,
      //     height: 32,
      //   }
      ...
      // $log.log(icon);
      // }
      // aqui agregar lugarres
      map.addMarker({
        place: place,
        position: setPosition(place.gpslat, place.gpslon),
        title: place.titulo,
        icon: icon,
        ...
        infoClick: function (marker) {
          $state.go('app.place', { placeId: marker.get('placeId'), 'obj': place});
        }
      }, function (marker) {
        markers.push(marker);
      });
